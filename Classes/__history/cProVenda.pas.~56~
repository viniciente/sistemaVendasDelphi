...
uses System.Classes, Vcl.Controls, Vcl.ExtCtrls, Vcl.Dialogs,
     FireDAC.Stan.Intf, FireDAC.Stan.Option,
      ...
      System.SysUtils, Datasnap.DBClient, FireDAC.Stan.Param, uEnum, cControleEstoque,
      cCadCliente;
...
{$REGION 'Constructro and destructor'}
...
begin
  try
    ...
    try
      ...
      //apagar o ClientDataSet caso esteja no registro
      while not cds.Eof do begin
        cds.Delete;
      end;

      //Selecione os Itens do banco de daos com a propiedades F_VendaId
      Qry.Close;
      Qry.SQL.Clear;
      ...
      //le a query e coloca no ClientDatSet
      Qry.First;
...
begin
  ...
  try
    ...
    // Usa OUTPUT INSERTED para recuperar o ID gerado no SQL Server
    Qry.SQL.Clear;
    Qry.SQL.Add('INSERT INTO vendas (clienteId, dataVenda, totalVenda) ');
    Qry.SQL.Add('OUTPUT INSERTED.vendaId ');
    Qry.SQL.Add('VALUES (:clienteId, :dataVenda, :totalVenda)');
    Qry.ParamByName('clienteId').AsInteger  :=Self.F_clienteId;
    Qry.ParamByName('dataVenda').AsDateTime :=Self.F_dataVenda;
    Qry.ParamByName('totalVenda').AsFloat   :=Self.F_totalVenda;
    ...
    Try
      ...
      if IdVendaGerado <= 0 then begin
        FdConexao.Rollback;
        ShowMessage('Erro: nao foi possivel obter o ID da venda gerada.');
        Result := -1;
        Exit;
      end;
      ...
      oCliente := TCliente.Create(FdConexao);
      try
        // Selecionamos o cliente da venda para carregar o status atual dele
        if oCliente.Selecionar(Self.F_clienteId) then
        begin
          // Chama o método que criamos na classe TCliente
          oCliente.AtualizarStatusParaAtivo;
        end;
      finally
        oCliente.Free;
      end;
...
begin
    ...
    Qry.ParamByName('vendaId').AsInteger    :=Self.F_vendaId;
    Try
      oControleEstoque:=TControleEstoque.Create(FdConexao);
      Qry.Open;
      Qry.First;
      while not Qry.Eof do begin
