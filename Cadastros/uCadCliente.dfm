inherited frmCadCliente: TfrmCadCliente
  Caption = 'Cadastro de Cliente'
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnlFundo: TPanel
    inherited pnlRodape: TPanel
      inherited dbnvgrNavigator: TDBNavigator
        Hints.Strings = ()
      end
    end
    inherited pgcPrincipal: TPageControl
      inherited tsListagem: TTabSheet
        inherited pnlListagemTopo: TPanel
          inherited maskPesquisar: TMaskEdit
            Width = 225
            ExplicitWidth = 225
          end
          inherited btnPesquisar: TBitBtn
            Left = 231
            ExplicitLeft = 231
          end
          object Panel1: TPanel
            Left = 312
            Top = 23
            Width = 415
            Height = 21
            TabOrder = 2
            object Image1: TImage
              Left = 4
              Top = 0
              Width = 20
              Height = 20
              Center = True
              Picture.Data = {
                
                FFFF}
              OnClick = Image1Click
            end
            object lblStatusAtivo: TLabel
              Left = 30
              Top = 4
              Width = 35
              Height = 13
              Caption = 'ATIVO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object Image2: TImage
              Left = 71
              Top = 1
              Width = 20
              Height = 20
              Center = True
              Picture.Data = {
                
                FFFF}
              OnClick = Image2Click
            end
            object Image3: TImage
              Left = 166
              Top = 1
              Width = 20
              Height = 20
              Center = True
              Picture.Data = {
                
                FFFF}
              OnClick = Image3Click
            end
            object Image4: TImage
              Left = 246
              Top = 1
              Width = 20
              Height = 20
              Center = True
              Picture.Data = {
                
                FFFF}
              OnClick = Image4Click
            end
            object Image5: TImage
              Left = 318
              Top = 1
              Width = 20
              Height = 20
              Center = True
              Picture.Data = {
                
                FFFF}
              OnClick = Image5Click
            end
            object lblStatusBloqueado: TLabel
              Left = 95
              Top = 4
              Width = 67
              Height = 13
              Caption = 'BLOQUEADO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object lblStatusAtencao: TLabel
              Left = 191
              Top = 4
              Width = 51
              Height = 13
              Caption = 'ATEN'#199#195'O'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object lblStatusInativo: TLabel
              Left = 266
              Top = 4
              Width = 47
              Height = 13
              Caption = 'INATIVO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object lblStatusProspecto: TLabel
              Left = 340
              Top = 4
              Width = 65
              Height = 13
              Caption = 'PROSPECTO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = [fsBold]
              ParentFont = False
            end
          end
        end
        inherited dbgrdListagem: TDBGrid
          DataSource = dsListagem
          Columns = <
            item
              Expanded = False
              FieldName = 'clienteId'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'nome'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'statusId'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'cpf_cnpj'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'numero'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'status'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'tipoPessoa'
              Visible = True
            end>
        end
      end
      inherited tsManutencao: TTabSheet
        object lblCPFCNPJ: TLabel
          Left = 280
          Top = 139
          Width = 54
          Height = 13
          Caption = 'Documento'
        end
        object lblCodigo: TLabel
          Left = 24
          Top = 13
          Width = 33
          Height = 13
          Caption = 'Codigo'
        end
        object lblNome: TLabel
          Left = 26
          Top = 59
          Width = 27
          Height = 13
          Caption = 'Nome'
        end
        object lblEmail: TLabel
          Left = 26
          Top = 99
          Width = 24
          Height = 13
          Caption = 'Email'
        end
        object lblCEP: TLabel
          Left = 26
          Top = 139
          Width = 19
          Height = 13
          Caption = 'CEP'
        end
        object lblEndereco: TLabel
          Left = 26
          Top = 175
          Width = 45
          Height = 13
          Caption = 'Endere'#231'o'
        end
        object lblBairro: TLabel
          Left = 26
          Top = 211
          Width = 28
          Height = 13
          Caption = 'Bairro'
        end
        object lblCidade: TLabel
          Left = 26
          Top = 251
          Width = 33
          Height = 13
          Caption = 'Cidade'
        end
        object lblEstado: TLabel
          Left = 26
          Top = 288
          Width = 33
          Height = 13
          Caption = 'Estado'
        end
        object lblNumero: TLabel
          Left = 28
          Top = 324
          Width = 37
          Height = 13
          Caption = 'Numero'
        end
        object lblTelefone: TLabel
          Left = 280
          Top = 59
          Width = 42
          Height = 13
          Caption = 'Telefone'
        end
        object lblTipoPessoa: TLabel
          Left = 280
          Top = 99
          Width = 57
          Height = 13
          Caption = 'Tipo Pessoa'
        end
        object lblStatus: TLabel
          Left = 280
          Top = 175
          Width = 31
          Height = 13
          Caption = 'Status'
        end
        object lblData: TLabel
          Left = 280
          Top = 211
          Width = 23
          Height = 13
          Caption = 'Data'
        end
        object edtClienteId: TEdit
          Left = 24
          Top = 32
          Width = 121
          Height = 21
          Enabled = False
          TabOrder = 0
        end
        object edtNome: TEdit
          Left = 24
          Top = 72
          Width = 225
          Height = 21
          TabOrder = 1
          OnKeyPress = edtNomeKeyPress
        end
        object edtEmail: TEdit
          Left = 24
          Top = 112
          Width = 225
          Height = 21
          TabOrder = 2
        end
        object edtCEP: TEdit
          Left = 24
          Top = 152
          Width = 225
          Height = 21
          MaxLength = 9
          TabOrder = 3
          OnChange = edtCEPChange
          OnExit = edtCEPExit
          OnKeyPress = edtCEPKeyPress
        end
        object edtEndereco: TEdit
          Left = 24
          Top = 187
          Width = 225
          Height = 21
          Enabled = False
          TabOrder = 4
        end
        object edtBairro: TEdit
          Left = 24
          Top = 224
          Width = 225
          Height = 21
          Enabled = False
          TabOrder = 5
        end
        object edtCidade: TEdit
          Left = 24
          Top = 264
          Width = 225
          Height = 21
          Enabled = False
          TabOrder = 6
        end
        object edtNumero: TEdit
          Left = 24
          Top = 336
          Width = 225
          Height = 21
          TabOrder = 8
          OnChange = edtNumeroChange
          OnKeyPress = edtNumeroKeyPress
        end
        object edtEstado: TEdit
          Left = 24
          Top = 301
          Width = 225
          Height = 21
          Enabled = False
          MaxLength = 2
          TabOrder = 7
        end
        object edtTelefone: TEdit
          Left = 280
          Top = 72
          Width = 225
          Height = 21
          TabOrder = 9
          OnChange = edtTelefoneChange
          OnKeyPress = edtTelefoneKeyPress
        end
        object edtDataNascimento: TDateEdit
          Left = 280
          Top = 224
          Width = 113
          Height = 21
          ClickKey = 114
          DialogTitle = 'Selecione a Data'
          NumGlyphs = 2
          CalendarStyle = csDialog
          TabOrder = 10
        end
        object edtCPFCNPJ: TEdit
          Left = 280
          Top = 152
          Width = 225
          Height = 21
          TabOrder = 11
          OnChange = edtCPFCNPJChange
          OnKeyPress = edtCPFCNPJKeyPress
        end
        object lkpPessoa: TDBLookupComboBox
          Left = 280
          Top = 112
          Width = 225
          Height = 21
          KeyField = 'pessoaId'
          ListField = 'descricao'
          ListSource = dsPessoa
          TabOrder = 12
          OnClick = lkpPessoaClick
        end
        object lpkStatus: TDBLookupComboBox
          Left = 280
          Top = 187
          Width = 225
          Height = 21
          KeyField = 'statusId'
          ListField = 'descricao'
          ListSource = dsStatus
          TabOrder = 13
          OnClick = lpkStatusClick
        end
        object pnlImage: TPanel
          Left = 511
          Top = 187
          Width = 21
          Height = 21
          BevelOuter = bvNone
          TabOrder = 14
          object imgStatus: TImage
            Left = 0
            Top = 0
            Width = 21
            Height = 21
            Align = alClient
            Center = True
            Proportional = True
            ExplicitLeft = 8
            ExplicitTop = 16
            ExplicitWidth = 105
            ExplicitHeight = 105
          end
        end
      end
    end
  end
  object qryStatus: TFDQuery [1]
    Connection = dtmConexao.fdConexao
    SQL.Strings = (
      'SELECT statusId,'
      '       descricao'
      'FROM statusClientes')
    Left = 540
    object qryStatusstatusId: TFDAutoIncField
      FieldName = 'statusId'
      Origin = 'statusId'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object qryStatusdescricao: TStringField
      FieldName = 'descricao'
      Origin = 'descricao'
      Required = True
    end
  end
  object dsStatus: TDataSource [2]
    DataSet = qryStatus
    Left = 620
  end
  object qryPessoa: TFDQuery [3]
    Active = True
    Connection = dtmConexao.fdConexao
    SQL.Strings = (
      'SELECT pessoaId,'
      '       descricao'
      'from tipoPessoa'
      'ORDER BY descricao')
    Left = 500
    object qryPessoapessoaId: TFDAutoIncField
      FieldName = 'pessoaId'
      Origin = 'pessoaId'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object qryPessoadescricao: TStringField
      FieldName = 'descricao'
      Origin = 'descricao'
      Required = True
      Size = 15
    end
  end
  object dsPessoa: TDataSource [4]
    DataSet = qryPessoa
    Left = 588
  end
  object imglStatus: TImageList [5]
    Left = 404
    Bitmap = {
      
      000000000000}
  end
  inherited dsListagem: TDataSource
    OnDataChange = dsListagemDataChange
    Left = 672
    Top = 0
  end
  inherited FDQuery1: TFDQuery
    SQL.Strings = (
      'SELECT '
      '    cl.clienteId, '
      '    cl.nome, '
      '    cl.endereco, '
      '    cl.cidade, '
      '    cl.bairro, '
      '    cl.estado, '
      '    cl.cep, '
      '    cl.telefone, '
      '    cl.email, '
      '    cl.dataNascimento, '
      '    cl.statusId, '
      '    cl.pessoaId, '
      '    cl.cpf_cnpj, '
      '    cl.numero,'
      '    s.descricao AS status, '
      '    t.descricao AS tipoPessoa'
      'FROM clientes cl'
      'INNER JOIN statusClientes s ON s.statusId = cl.statusId'
      'INNER JOIN tipoPessoa t ON t.pessoaId = cl.pessoaId'
      'ORDER BY cl.nome')
    Left = 444
    Top = 0
    object FDQuery1clienteId: TFDAutoIncField
      DisplayLabel = 'Codigo Cliente'
      FieldName = 'clienteId'
      Origin = 'clienteId'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object FDQuery1nome: TStringField
      DisplayLabel = 'Nome'
      FieldName = 'nome'
      Origin = 'nome'
      Size = 60
    end
    object FDQuery1endereco: TStringField
      DisplayLabel = 'Endereco'
      FieldName = 'endereco'
      Origin = 'endereco'
      Size = 60
    end
    object FDQuery1cidade: TStringField
      DisplayLabel = 'Cidade'
      FieldName = 'cidade'
      Origin = 'cidade'
      Size = 50
    end
    object FDQuery1bairro: TStringField
      DisplayLabel = 'Bairro'
      FieldName = 'bairro'
      Origin = 'bairro'
      Size = 40
    end
    object FDQuery1estado: TStringField
      DisplayLabel = 'Estado'
      FieldName = 'estado'
      Origin = 'estado'
      Size = 2
    end
    object FDQuery1cep: TStringField
      DisplayLabel = 'CEP'
      FieldName = 'cep'
      Origin = 'cep'
      OnGetText = FDQuery1cepGetText
      Size = 10
    end
    object FDQuery1telefone: TStringField
      DisplayLabel = 'Telefone'
      FieldName = 'telefone'
      Origin = 'telefone'
      OnGetText = FDQuery1telefoneGetText
      Size = 14
    end
    object FDQuery1email: TStringField
      DisplayLabel = 'Email'
      FieldName = 'email'
      Origin = 'email'
      Size = 100
    end
    object FDQuery1dataNascimento: TSQLTimeStampField
      DisplayLabel = 'Data Nascimento'
      FieldName = 'dataNascimento'
      Origin = 'dataNascimento'
    end
    object FDQuery1statusId: TIntegerField
      DisplayLabel = 'Status'
      FieldName = 'statusId'
      Origin = 'statusId'
    end
    object FDQuery1pessoaId: TIntegerField
      DisplayLabel = 'Tipo Pessoa'
      FieldName = 'pessoaId'
      Origin = 'pessoaId'
    end
    object FDQuery1cpf_cnpj: TStringField
      DisplayLabel = 'Documento'
      FieldName = 'cpf_cnpj'
      Origin = 'cpf_cnpj'
      OnGetText = FDQuery1cpf_cnpjGetText
    end
    object FDQuery1numero: TStringField
      DisplayLabel = 'Numero'
      FieldName = 'numero'
      Origin = 'numero'
      Size = 10
    end
    object FDQuery1status: TStringField
      DisplayLabel = 'Status'
      FieldName = 'status'
      Origin = 'status'
      Required = True
    end
    object FDQuery1tipoPessoa: TStringField
      DisplayLabel = 'Tipo Pessoa'
      FieldName = 'tipoPessoa'
      Origin = 'tipoPessoa'
      Required = True
      Size = 15
    end
  end
end
