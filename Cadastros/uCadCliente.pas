...
uses
  ...
  System.JSON, System.RegularExpressions, System.Character;
...
type
  TfrmCadCliente = class(TfrmTelaHeranca)
    ...
    lblStatus: TLabel;
    lblData: TLabel;
    FDQuery1clienteId: TFDAutoIncField;
...
begin
  ...
  begin
    ...
    begin
      MessageDlg('CPF Inválido! Por favor, verifique os números.', mtError, [mbOK], 0);
      edtCPFCNPJ.SetFocus;
      Result := False;
      Exit;
    end;
  ...
  //validação do CNPJ
  else if lkpPessoa.KeyValue = 2 then // Pessoa Jurídica
  begin
  ...
  // Se o código chegou aqui, significa que o CPF/CNPJ é válido.
...
begin
  ...
  // Limpa máscara (deixa só números)
    ...
    try
      ...
      begin
        ...
        try
          ...
          begin
            // A API do ViaCEP retorna "erro: true" se o CEP for válido mas não existir
...
begin
  // Ignora teclas de controle (setas, backspace, etc) para não atrapalhar a navegação
...
begin
  edtCPFCNPJ.Clear; //limpa o campo para não ficar com numero caso mude de pessoa
  ...
  edtCPFCNPJ.SetFocus; //foco fica no edit pra começar a digitar
end;
...
begin
  ...
  begin
    ...
    // Se o imgStatus estiver dentro de um Panel1, ele usará a cor do Panel1
